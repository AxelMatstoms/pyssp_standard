...
class AnalysisPhase(GPhaseCommon, BaseElement, ModelicaStandard):
    analyze_modeling_task_and_objectives: Step
...
class FulfillmentPhase(GPhaseCommon, BaseElement, ModelicaStandard):
    decide_modeling_objective_fulfillment: Step


class MTMD(TaskMetaData):
    ...
    def __init__(self, file_path, mode="r"):
        ...
        super().__init__(file_path, mode, "mtmd11")

    def __read__(self):
        super().__read__()

        fields = MTMD.__annotations__
        for name, cls in fields.items():
            if (elem := self.root.find(f"mtmd:{cls.__name__}", self.namespaces)) is not None:
